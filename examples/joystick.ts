import { createWindow, mainloop, joystickPresent, getJoystickAxes,getJoystickButtons } from "../mod.ts";
...
await mainloop(() => {
    const i = joystickPresent(0);
    if (i) {
        //console.log('joystickPresent: ', 0)
        const axies = getJoystickAxes(0);

        const key = axies.join(':');
